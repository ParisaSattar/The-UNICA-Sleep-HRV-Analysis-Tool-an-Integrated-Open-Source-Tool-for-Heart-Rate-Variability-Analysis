...
ploting_segments = uicontrol('Parent',tab1,'style', 'popupmenu',...
    ...
    'Enable','off','position', [0.39 0.46 0.07 0.15]);

% Unit popup menu
Unit = uicontrol('Parent',tab1,'style', 'popupmenu',...
    'string', {'Samples','Seconds',},...
    'units', 'normalized','FontSize', 10,...
    'position', [0.53 0.72 0.1 0.15],...
    'Enable','off');
    ...
    function plot_segment(object, event)
        ...
        if segment_Number<= sl
            ...
            if Unit.Value==1
                ...
                xlabel('Samples')
                if max(abs(ECG_data)) < 10  % Likely in mV
                    ylabel(axes1, 'ECG (mV)');
                else  % Likely in µV
                    ylabel(axes1, 'ECG (µV)');
                end
        ...
        if control==1||control==3
            if Unit.Value==1
                ...
                set(axes1, 'XGrid', 'on','YGrid', 'on')
                  ...
                  if max(abs(ECG)) < 10  % Likely in mV
                      ylabel(axes1, 'ECG (mV)');
                  else  % Likely in µV
                      ylabel(axes1, 'ECG (µV)');
                  end
...
%Callback function 2 for Rpeak correction start here
    function plot_ButtonDownFcn(object, event)
        if isInEditMode==1
            ...
            elseif Unit.Value == 2
                currentPoint = get(gca, 'CurrentPoint');
                xClick = currentPoint(1,1)*fs;
                 xClick1 = currentPoint(1,1);% X-coordinate of the click
                t1 = (0:length(ECG_data)-1)/fs;
                [~, x_sample] = min(abs(t1 - xClick1));
                % Determine if the click is near an existing marker
                threshold = 20; % Define threshold for "closeness"
                distances = abs(R_pos - xClick);
                [minDistance, nearestIndex] = min(distances);
